...
/**
 * 目的：
 * - 月曜はじまりのカレンダーで日付を選ぶ（祝日処理なし）
 * - 30分刻み（24時間制）で「帯」をドラッグして時間範囲を作成
 * - いったん離した後でも、上下端をドラッグしてその場で微調整（伸ばす/縮める）
 * - ドラッグ中は「HH:mm–HH:mm」をリアルタイム表示
 * - 複数日・複数枠に対応（上限は設けない）
 * - 候補文をテンプレに差し込んでコピー
 *
 * 注意：
 * - 依存は React + Tailwind のみ（package.json で既に導入済み）
 */
...
export default function App() {
  ...
  const outputText = useMemo(() => {
    ...
    return template
      .replaceAll("{{宛先名}}", name)
      .replaceAll("{{候補一覧}}", candidateListText);
...
{candidateListText}
        ...
        <div className="bg-white rounded-xl shadow p-3 mb-4">
          ...
          <label className="block text-sm font-medium mb-1">テンプレ（{{宛先名}} / {{候補一覧}} を差し込み）</label>
          <textarea
